pipeline {
  ...
  environment {
    ...
    CONTAINER_PORT = "8080"
    HOST_PORT = "${8000 + BUILD_NUMBER % 1000}"
  }
  stages {
    stage('Checkout') {
      steps {
      ...
      steps {
        script {
          sh '''
            ...
            curl -f http://localhost:${HOST_PORT}/actuator/health || echo "Health endpoint not available"
          '''
        }
      }
    }
  ...
  post {
    success { 
      echo "SUCCESS: App deployed at http://localhost:${HOST_PORT}"
    }
    failure { 
